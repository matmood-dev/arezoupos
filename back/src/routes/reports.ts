 ...
 */
router.get('/', authenticateToken, requireAdmin, async (req: Request, res: Response<ApiResponse>) => {
  try {
    // Totals
    ...
    const topItemsResult = await query(
      `SELECT i.itemid, i.name, COALESCE(SUM(oi.quantity), 0) AS total_quantity
       FROM order_items oi
    ...
    const salesByDayResult = await query(
      `SELECT DATE(o.created_at) AS date, COALESCE(SUM(o.total_amount), 0) AS total_sales
       FROM orders o
