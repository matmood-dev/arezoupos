...
import { ApiResponse, Settings, UpdateSettingsRequest, Branch, CreateBranchRequest, UpdateBranchRequest, ItemCategory, CreateCategoryRequest } from '../types';

// Simple validation functions
const validateSettingsUpdate = (data: UpdateSettingsRequest) => {
  const errors: string[] = [];
...
const validateBranchCreate = (data: CreateBranchRequest) => {
  const errors: string[] = [];
...
const validateBranchUpdate = (data: UpdateBranchRequest) => {
  const errors: string[] = [];
...
const validateCategoryCreate = (data: CreateCategoryRequest) => {
  const errors: string[] = [];
...
const logoFileFilter = (req: express.Request, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
...
router.get('/', authenticateToken, async (req, res): Promise<void> => {
  try {
    ...
    const settings = result.rows[0] as Settings;
...
// Update settings
router.put('/', express.json(), upload.any(), authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    ...
    const currentLogo = currentSettingsResult.rows.length > 0 ? (currentSettingsResult.rows[0] as any).shop_logo : null;
    ...
    // Handle logo upload
    const logoFile = (req.files as Express.Multer.File[])?.find((file: Express.Multer.File) => file.fieldname === 'shop_logo');
    if (logoFile) {
    ...
    const result = await query('SELECT * FROM settings WHERE settingid = 1');
    const updatedSettings = result.rows[0] as Settings;
...
router.get('/branches', authenticateToken, async (req, res): Promise<void> => {
  try {
    const result = await query('SELECT * FROM branches ORDER BY name');
    const branches = result.rows as Branch[];
...
// Create branch
router.post('/branches', express.json(), authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    ...
    const newBranch = newBranchResult.rows[0] as Branch;
...
// Update branch
router.put('/branches/:id', express.json(), authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    const branchId = parseInt(req.params.id || '0');
    const updateData: UpdateBranchRequest = req.body;
    ...
    Object.entries(updateData).forEach(([key, value]) => {
      if (value !== undefined) {
        ...
        if (key === 'active') {
          ...
          if (typeof v === 'boolean') processedValue = v;
          else if (typeof v === 'string') processedValue = (v === 'true' || v === '1');
          else if (typeof v === 'number') processedValue = (v === 1);
        }
    ...
    const updatedBranch = updatedResult.rows[0] as Branch;
...
// Delete branch
router.delete('/branches/:id', authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    const branchId = parseInt(req.params.id || '0');
...
router.get('/categories', authenticateToken, async (req, res) => {
  try {
    ...
    const categories = result.rows as ItemCategory[];

    res.json({
      success: true,
      data: categories
    } as ApiResponse<ItemCategory[]>);
  } catch (error) {
...
// Create category
router.post('/categories', express.json(), authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
    ...
    const newCategory = newCategoryResult.rows[0] as ItemCategory;
...
// Delete category
router.delete('/categories/:id', authenticateToken, requireAdmin, async (req, res): Promise<void> => {
  try {
