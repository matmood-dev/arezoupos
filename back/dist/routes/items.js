...
import { validateIdParam } from '../validators/index.js';
 ...
 *       - bearerAuth: []
 */
router.put('/:itemid', authenticateToken, requireAdminForDelete, upload.single('image'), validateIdParam, async (req, res) => {
    try {
        const { itemid } = req.params;
        const { name, description, price, category, stock_quantity } = req.body;
    ...
    `, [itemid]);
        ...
        const currentImage = currentItemResult.rows[0].image;
        const updates = [];
        const values = [];
        if (name !== undefined) {
            updates.push(`name = ?`);
