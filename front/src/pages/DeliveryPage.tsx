...
/* Title (unused after refactor) removed */
...
/* Old Th/Td definitions removed in favor of invoice-style TableHeader/TableCell */
...
/* Old totals row/label/amount removed; using TotalsBox instead */
...
const DeliveryPage: React.FC = () => {
  ...
  if (loading || !order) return <PageContainer><p>Loading delivery note...</p></PageContainer>;
  // totals
  const taxRate = settings?.tax_rate ? Number(settings.tax_rate) : 0;
  const computedSubtotal = order.items.reduce((s, it) => s + (Number(it.price || 0) * (it.quantity || 0)), 0);
  ...
  return (
    <PageContainer>
          ...
          <PrintButton onClick={async () => {
            try {
              ...
              await html2pdf().from(clonedEl).set({
                filename: `delivery-${order.orderid}.pdf`,
                margin: [10, 10, 10, 10],
                html2canvas: { scale: 2 },
      ...
      <Wrapper id="delivery-note">
        <HeaderRow>
          <CompanyInfo>
            {settings?.shop_logo && (
              <div style={{ marginBottom: 16 }}>
                <img src={settings.shop_logo} alt="logo" style={{ maxWidth: 150, maxHeight: 80, objectFit: 'contain' }} />
              </div>
            )}
          ...
          <RightColumn>
            <div style={{ textAlign: 'right' }}>
              <InvoiceTitleText>DELIVERY NOTE</InvoiceTitleText>
            </div>
          ...
          <CustomerInfo>
            ...
            <div style={{ fontSize: 14, lineHeight: 1.4 }}>
              {(() => {
                ...
                return <div><strong>Walk-in Customer</strong></div>;
              })()}
            </div>
          </CustomerInfo>
          <InvoiceDetails>
            <SectionTitle>Delivery Details</SectionTitle>
            <InfoRow><strong>Order:</strong> #{order.orderid}</InfoRow>
            <InfoRow><strong>Status:</strong> {order.status}</InfoRow>
          </InvoiceDetails>
        </CustomerSection>

        <Divider />

        <SectionTitle>Items Delivered</SectionTitle>
        <ItemsTable>
          <thead>
            <tr>
              ...
              return (
                <tr key={i}>
                  <TableCell>
                    ...
                    {it.note && <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4 }}>Note: {it.note}</div>}
                  </TableCell>
